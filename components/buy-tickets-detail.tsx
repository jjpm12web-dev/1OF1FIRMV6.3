...
export default function BuyTicketsDetail({ onNavigate }: BuyTicketsDetailProps) {
  ...
  // Each weekend event is independent: every card reflects its own soldOut value
  // from the store. The bottom ticket cards only sell out when every weekend event
  // is marked as SOLD OUT in the admin panel.
  ...
  // Visual styling per card position (color/style accents not stored in the data model)
  const cardStyles = [
    { dayLabelColor: "bg-amber-500", subtitleStyle: "script" as const, buttonStyle: "outline" as const },
    { dayLabelColor: "bg-amber-600", subtitleStyle: "italic" as const, buttonStyle: "outline" as const },
    { dayLabelColor: "bg-amber-700", subtitleStyle: "script" as const, buttonStyle: "filled" as const },
  ]
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="px-2 sm:px-3 md:px-8 pb-6 sm:pb-8 space-y-3 sm:space-y-4">
        {weekendEventsData.map((event, index) => {
          const cardSoldOut = event.soldOut ?? false
            ...
            <div className="flex flex-col sm:flex-row">
              ...
              <div className="p-3 sm:p-4 md:p-6 flex-1">
                ...
                <div className="inline-block mb-2 sm:mb-3">
                  {event.dayLabel && (
                    <span className={`${style.dayLabelColor} text-black text-[8px] sm:text-[10px] md:text-xs font-bold px-1.5 sm:px-2 py-0.5 sm:py-1 rounded tracking-wider`}>
                      {event.dayLabel}
                    </span>
                  )}
                </div>
                ...
                <div className="mb-2 sm:mb-3">
                  ...
                  <span className={`text-xs sm:text-sm md:text-3xl ${style.subtitleStyle === "script" ? "font-serif italic" : "italic font-light"} text-amber-500`}>
                    {event.subtitle}
                  </span>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-2 sm:mb-3 md:mb-4 text-[8px] sm:text-[10px] md:text-xs text-white/60">
                  ...
                  {(event.venue || event.location) && (
                    <div className="flex items-center gap-1 sm:gap-1.5">
                      ...
                      <span className="tracking-wider">{[event.venue, event.location].filter(Boolean).join(", ")}</span>
                    </div>
                  )}
